import time
import requests
from typing import Dict, List, Optional, Any, Iterator, Callable, Awaitable, Tuple
...
from .models import Character, Location, Episode, CharacterResponse, LocationResponse, EpisodeResponse
...
class RickMortyService:
    ...
    def __init__(self, timeout: int = None, max_retries: int = None, base_delay: float = None):
    ...
    def _sort_characters_stable(self, characters: List[Dict[str, Any]], sort_by: str = "name", sort_order: str = "asc") -> List[Dict[str, Any]]:
        ...
        def stable_sort_key(char):
            ...
            secondary = char.get("id", 0)  # Use ID as tiebreaker for stable sorting
            return (primary, secondary)
        
        reverse = sort_order.lower() == "desc"
        return sorted(characters, key=stable_sort_key, reverse=reverse)
    
    def _make_request_with_retry(self, url: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                metrics_collector.record_external_api_call("rick_morty_api", "success", duration)
                ...
                if attempt < self.max_retries:
                    ...
                    duration = time.time() - start_time
                    metrics_collector.record_external_api_call("rick_morty_api", "error", duration)
                    break
        
        raise Exception(f"API request failed after {self.max_retries + 1} attempts: {str(last_exception)}")
    
    def _iter_pages(self, endpoint: str, params: Optional[Dict[str, Any]] = None, max_pages: Optional[int] = None) -> Iterator[Dict[str, Any]]:
        ...
        while url and (max_pages is None or page_count < max_pages):
            try:
                ...
                logger.warning(f"Failed to fetch page {page_count + 1} from {url}: {str(e)}")
                break  # Stop pagination on error
    
    def _build_character_filters(self, name: Optional[str] = None, status: Optional[str] = None,
                                species: Optional[str] = None, type: Optional[str] = None,
                                gender: Optional[str] = None) -> Dict[str, str]:
        ...
        if status:
            ...
            if not self._validate_enum(normalized_status, CharacterStatus):
                raise ValueError(f"Invalid status: {status}. Must be one of: {[s.value for s in CharacterStatus]}")
        ...
        if gender:
            ...
            if not self._validate_enum(normalized_gender, CharacterGender):
                raise ValueError(f"Invalid gender: {gender}. Must be one of: {[g.value for g in CharacterGender]}")
            params['gender'] = normalized_gender
        
        return params
    
    def get_characters(self, page: Optional[int] = None, name: Optional[str] = None,
                       status: Optional[str] = None, species: Optional[str] = None,
                       type: Optional[str] = None, gender: Optional[str] = None,
                       sort_by: str = "name", sort_order: str = "asc") -> CharacterResponse:
        """Get characters with optional filtering, pagination, and stable sorting"""
        if page:
            # Single page request
            params = self._build_character_filters(name, status, species, type, gender)
            params['page'] = page
            data = self._make_request_with_retry(f"{self.base_url}/character", params)
            
            # Apply stable sorting to single page results
            sorted_results = self._sort_characters_stable(data.get("results", []), sort_by, sort_order)
            ...
            sorted_results = self._sort_characters_stable(all_results, sort_by, sort_order)
    ...
    def get_all_characters(self, name: Optional[str] = None, status: Optional[str] = None,
                          species: Optional[str] = None, type: Optional[str] = None,
                          gender: Optional[str] = None, max_pages: int = 10,
                          sort_by: str = "name", sort_order: str = "asc") -> List[Character]:
    ...
    def fetch_alive_earth_humans(self) -> List[Character]:
        ...
        try:
            ...
            for character_data in self._iter_pages("character", {"species": "Human", "status": "Alive"}):
    ...
    def get_character_by_id(self, character_id: int) -> Character:
        ...
        data = self._make_request_with_retry(f"{self.base_url}/character/{character_id}")
        return Character(**data)
    
    def get_locations(self, page: Optional[int] = None, name: Optional[str] = None,
                     type: Optional[str] = None, dimension: Optional[str] = None) -> LocationResponse:
        ...
        if page:
            data = self._make_request_with_retry(f"{self.base_url}/location", params)
    ...
    def get_all_locations(self, name: Optional[str] = None, type: Optional[str] = None,
                         dimension: Optional[str] = None, max_pages: int = 10) -> List[Location]:
        ...
        return [Location(**loc) for loc in self._iter_pages("location", params, max_pages)]
    
    def get_location_by_id(self, location_id: int) -> Location:
        """Get a specific location by ID with retry logic"""
        data = self._make_request_with_retry(f"{self.base_url}/location/{location_id}")
        return Location(**data)
    
    def get_episodes(self, page: Optional[int] = None, name: Optional[str] = None,
                    episode: Optional[str] = None) -> EpisodeResponse:
        ...
        if page:
            data = self._make_request_with_retry(f"{self.base_url}/episode", params)
    ...
    def get_all_episodes(self, name: Optional[str] = None, episode: Optional[str] = None, max_pages: int = 10) -> List[Episode]:
        ...
        return [Episode(**ep) for ep in self._iter_pages("episode", params, max_pages)]
    
    def get_episode_by_id(self, episode_id: int) -> Episode:
        """Get a specific episode by ID with retry logic"""
        data = self._make_request_with_retry(f"{self.base_url}/episode/{episode_id}")
        return Episode(**data)
    
    def get_multiple_characters(self, character_ids: List[int]) -> List[Character]:
        """Get multiple characters by their IDs with retry logic"""
        ids_str = ",".join(map(str, character_ids))
        data = self._make_request_with_retry(f"{self.base_url}/character/{ids_str}")
    ...
    def get_multiple_locations(self, location_ids: List[int]) -> List[Location]:
        """Get multiple locations by their IDs with retry logic"""
        ids_str = ",".join(map(str, location_ids))
        data = self._make_request_with_retry(f"{self.base_url}/location/{ids_str}")
    ...
    def get_multiple_episodes(self, episode_ids: List[int]) -> List[Episode]:
        ...
        data = self._make_request_with_retry(f"{self.base_url}/episode/{ids_str}")
    ...
    # Database operations
    async def save_character_to_db(self, db: AsyncSession, character_data: Dict[str, Any]) -> DBCharacter:
        """Save character data to database"""
        try:
            # Check if character already exists
            result = await db.execute(
                select(DBCharacter).where(DBCharacter.id == character_data["id"])
            )
            ...
            if existing_character:
                ...
                db_character = DBCharacter(
                    ...
                    is_earth_human=character_data["origin"]["name"].startswith("Earth"),
        ...
        try:
            ...
            if filters:
                if filters.get("status"):
                    query = query.where(DBCharacter.status == filters["status"])
                if filters.get("species"):
                    query = query.where(DBCharacter.species == filters["species"])
    ...
    async def get_character_from_db(self, db: AsyncSession, character_id: int) -> Optional[DBCharacter]:
    ...
    async def get_cached_characters(self, cache_key: str, refresh_func: Optional[Callable[[], Awaitable[List[Dict[str, Any]]]]] = None) -> Optional[List[Dict[str, Any]]]:
    ...
    async def cache_characters(self, cache_key: str, characters: List[Dict[str, Any]], ttl: int = None) -> bool:
    ...
    async def warm_cache_for_endpoint(self, endpoint: str, params: Dict[str, Any] = None):
        ...
        try:
            ...
            async def refresh_func():
                if endpoint == "character":
                    return await self._fetch_all_characters_from_api(params or {})
                elif endpoint == "location":
                    return await self._fetch_all_locations_from_api(params or {})
                elif endpoint == "episode":
                    return await self._fetch_all_episodes_from_api(params or {})
                return None
            
            await cache_service.warm_cache(cache_key, refresh_func)
            logger.info(f"Cache warming initiated for {endpoint} with params {params}")
        except Exception as e:
            logger.error(f"Error warming cache for {endpoint}: {e}")
    
    async def _fetch_all_characters_from_api(self, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Fetch all characters from API for cache warming"""
        try:
            all_results = []
            for character_data in self._iter_pages("character", params, max_pages=10):
    ...
    async def _fetch_all_locations_from_api(self, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Fetch all locations from API for cache warming"""
        try:
            all_results = []
            for location_data in self._iter_pages("location", params, max_pages=5):
    ...
    async def _fetch_all_episodes_from_api(self, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Fetch all episodes from API for cache warming"""
        try:
            all_results = []
            for episode_data in self._iter_pages("episode", params, max_pages=5):
    ...
    async def get_characters_paginated(
        ...
        try:
            ...
            if filters:
                if "name" in filters:
                    query = query.where(Character.name.ilike(f"%{filters['name']}%"))
                if "status" in filters:
                    query = query.where(Character.status == filters["status"])
                if "species" in filters:
                    query = query.where(Character.species.ilike(f"%{filters['species']}%"))
                if "type" in filters:
                    query = query.where(Character.type.ilike(f"%{filters['type']}%"))
    ...
    async def get_earth_humans_paginated(
        ...
        try:
            ...
            if total_count == 0 or force_refresh:
                await self.get_earth_humans_with_persistence(db, force_refresh=True)
                # Recalculate count
                total_count = await db.scalar(count_query)
    ...
    async def get_character_by_id(self, db: AsyncSession, character_id: int) -> Optional[Character]:
    ...
    async def get_location_by_id(self, db: AsyncSession, location_id: int) -> Optional[Location]:
    ...
    async def get_episode_by_id(self, db: AsyncSession, episode_id: int) -> Optional[Episode]:
    ...
    async def get_earth_humans_with_persistence(
        ...
        async def refresh_earth_humans():
            ...
            try:
                ...
                db_characters = await self.get_characters_from_db(
                    db, 
                    filters={"is_earth_human": True, "is_alive": True}
                )
                ...
                if db_characters:
                    ...
                    # Apply stable sorting
                    sorted_characters = self._sort_characters_stable(characters, sort_by="name", sort_order="asc")
                ...
                for character_data in self._iter_pages("character", {"species": "Human", "status": "Alive"}):
                    ...
                    if self._is_earth_variant(origin_name):
                        ...
                        try:
                            ...
                            logger.error(f"Failed to save character to database: {e}")
        ...
        if not force_refresh:
            cached_data = await self.get_cached_characters(cache_key, refresh_earth_humans)
            if cached_data:
