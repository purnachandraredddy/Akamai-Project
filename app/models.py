...
class Character(BaseModel):
    ...
    status: str = Field(..., description="Character status (Alive, Dead, unknown)")
    species: str = Field(..., description="Character species")
    type: str = Field("", description="Character type")
    gender: str = Field(..., description="Character gender (Male, Female, Genderless, unknown)")
    origin: Dict[str, Any] = Field(..., description="Character origin information")
    location: Dict[str, Any] = Field(..., description="Character location information")
    image: str = Field("", description="Character image URL")
    episode: List[str] = Field(default_factory=list, description="List of episode URLs")
...
class Location(BaseModel):
    ...
    residents: List[str] = Field(default_factory=list, description="List of resident character URLs")
...
class Episode(BaseModel):
    ...
    characters: List[str] = Field(default_factory=list, description="List of character URLs")
    ...
    def validate_episode_code(cls, v):
        ...
        if not re.match(r'^S\d{2}E\d{2}$', v.strip()):
            raise ValueError('Episode code must be in format S##E## (e.g., S01E01)')
        return v.strip()
...
class CharacterFilters(BaseModel):
    ...
    name: Optional[str] = Field(None, description="Filter by character name (partial match)")
    status: Optional[str] = Field(None, description="Filter by status (Alive, Dead, unknown)")
    species: Optional[str] = Field(None, description="Filter by species")
    type: Optional[str] = Field(None, description="Filter by type")
    gender: Optional[str] = Field(None, description="Filter by gender (Male, Female, Genderless, unknown)")
...
class LocationFilters(BaseModel):
    ...
    name: Optional[str] = Field(None, description="Filter by location name (partial match)")
...
class EpisodeFilters(BaseModel):
    ...
    name: Optional[str] = Field(None, description="Filter by episode name (partial match)")
    episode: Optional[str] = Field(None, description="Filter by episode code (e.g., S01E01)")
