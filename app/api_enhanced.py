...
@router.get(
    "/characters", response_model=SuccessResponse[PaginatedResponse[Character]]
)
@limiter.limit(
...
@router.get(
    "/characters/{character_id}", response_model=SuccessResponse[Character]
)
...
@router.get(
    "/locations", response_model=SuccessResponse[PaginatedResponse[Location]]
)
@limiter.limit(
    f"{settings.rate_limit_requests}/{settings.rate_limit_window} seconds"
)
...
@router.get(
    "/locations/{location_id}", response_model=SuccessResponse[Location]
)
...
@router.get(
    "/episodes", response_model=SuccessResponse[PaginatedResponse[Episode]]
)
@limiter.limit(
    f"{settings.rate_limit_requests}/{settings.rate_limit_window} seconds"
)
...
async def get_episode(
    episode_id: int, request: Request, db: AsyncSession = Depends(get_db)
):
...
@router.get(
    "/earth-humans",
    response_model=SuccessResponse[PaginatedResponse[Character]],
)
@limiter.limit(
    f"{settings.rate_limit_requests}/{settings.rate_limit_window} seconds"
)
