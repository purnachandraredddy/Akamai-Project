...
class CacheService:
    ...
    def __init__(self):
        ...
        self._refresh_locks: Dict[str, Tuple[asyncio.Lock, datetime]] = (
            {}
        )  # (lock, expiry)
        ...
        self._refresh_semaphore = asyncio.Semaphore(
            settings.cache_max_refresh_concurrency
        )
        self._refresh_inflight: Dict[str, int] = (
            {}
        )  # Track inflight refreshes per key group
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.warning(
                f"Redis not available, using L1 memory cache only: {e}"
            )
            self._redis_available = False
    ...
    async def get(
        ...
        try:
            ...
            if refresh_func:
                return await self._refresh_with_stampede_protection(
                    key, refresh_func
                )
    ...
    async def set(
        ...
        try:
            ...
            l1_jittered = l1_ttl + random.randint(
                0, int(l1_ttl * jitter_percent)
            )
            l2_jittered = l2_ttl + random.randint(
                0, int(l2_ttl * jitter_percent)
            )
    ...
    async def _set_l1(self, key: str, value: Any, ttl: int) -> bool:
        ...
        try:
            ...
            if value_size > settings.cache_l1_max_value_size:
                logger.debug(
                    f"Skipping L1 cache for large value: {value_size} bytes"
                )
                return False
    ...
    async def _schedule_refresh(
        self, key: str, refresh_func: Optional[Callable[[], Awaitable[Any]]]
    ):
    ...
    async def _refresh_with_stampede_protection(
        self, key: str, refresh_func: Callable[[], Awaitable[Any]]
    ) -> Optional[Any]:
        ...
        if key not in self._refresh_locks or self._refresh_locks[key][1] < now:
            self._refresh_locks[key] = (
                asyncio.Lock(),
                now + timedelta(seconds=self._lock_ttl),
            )
        ...
        async with lock:
            ...
            async with self._refresh_semaphore:
                try:
                    ...
                    self._refresh_inflight[key_group] = (
                        self._refresh_inflight.get(key_group, 0) + 1
                    )
                    ...
                    self._refresh_inflight[key_group] = max(
                        0, self._refresh_inflight.get(key_group, 0) - 1
                    )
                    self._metrics["refresh_inflight"] = max(
                        0, self._metrics["refresh_inflight"] - 1
                    )
    ...
    def _serialize_value(self, value: Any) -> bytes:
        ...
        try:
            ...
            if (
                ...
                and len(serialized) > self._compression_threshold
            ):
    ...
    def _generate_cache_key(
        self, endpoint: str, params: Dict[str, Any], version: str = None
    ) -> str:
    ...
    async def _cleanup_expired_locks(self):
        ...
        expired_keys = [
            key
            for key, (_, expiry) in self._refresh_locks.items()
            if expiry < now
        ]
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            l1_hit_ratio = (
                self._metrics["l1_hits"] / l1_total if l1_total > 0 else 0
            )
            l2_hit_ratio = (
                self._metrics["l2_hits"] / l2_total if l2_total > 0 else 0
            )
            ...
            health_info = {
                ...
                "l2_cache": {
                    ...
                    "type": (
                        "redis" if self._redis_available else "unavailable"
                    ),
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_evictions_total": {"l1": self._metrics["l1_evictions"]},
