...
from .security import add_no_cache_headers, require_admin_auth
...
async def get_characters(
    ...
    page: Optional[int] = Query(
        None, description="Page number (if not provided, returns all pages)"
    ),
...
async def get_all_characters(
    ...
    max_pages: int = Query(
        10, description="Maximum number of pages to fetch (default: 10)"
    ),
    sort_by: str = Query(
        "name", description="Sort by field (name, id, status, species)"
    ),
    sort_order: str = Query("asc", description="Sort order (asc, desc)"),
):
...
async def get_alive_earth_humans(
    ...
    force_refresh: bool = Query(
        False, description="Force refresh from external API"
    ),
    db: AsyncSession = Depends(get_db),
):
    """Get all alive humans from Earth with database persistence and caching"""
    try:
        return await service.get_earth_humans_with_persistence(
            db, force_refresh
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def warm_cache(
    ...
    response: Response,
    endpoint: str = Query(
        ..., description="Endpoint to warm (character, location, episode)"
    ),
    params: Optional[str] = Query(
        None, description="JSON string of parameters to filter by"
    ),
):
    ...
    try:
        ...
        return {
            "message": f"Cache warming initiated for {endpoint}",
...
@router.get(
    "/characters/multiple/{character_ids}", response_model=List[Character]
)
async def get_multiple_characters(character_ids: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Invalid character IDs format"
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_locations(
    ...
    try:
        return service.get_locations(
            page=page, name=name, type=type, dimension=dimension
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_all_locations(
    ...
    try:
        return service.get_all_locations(
            name=name, type=type, dimension=dimension, max_pages=max_pages
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
@router.get(
    "/locations/multiple/{location_ids}", response_model=List[Location]
)
async def get_multiple_locations(location_ids: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Invalid location IDs format"
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_all_episodes(
    ...
    try:
        return service.get_all_episodes(
            name=name, episode=episode, max_pages=max_pages
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_multiple_episodes(episode_ids: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Invalid episode IDs format"
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
